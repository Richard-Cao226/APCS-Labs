...
public class JMCh20Exercises<E>
{
    ...
    public ListNode<E> reverseList( ListNode<E> head )
    {
        ...
        if (head == null) {
            ...
            ListNode<E> newNode = new ListNode<E>(node.getValue(), null);
            for (;node.getNext() != null;) {
                node = node.getNext();
                ListNode<E> temp = new ListNode<E>(node.getValue(), newNode);
                newNode = temp;
            }
            return newNode;
        }
    ...
    public ListNode<String> concatenateStrings( ListNode<String> head )
    {
        ...
        if (head == null) {
            ...
            ListNode<String> newNode = new ListNode<String>(node.getValue(), null);
            ListNode<String> save = newNode;
            for (;node.getNext() != null;) {
                node = node.getNext();
                ListNode<String> temp = new ListNode<String>(newNode.getValue() + node.getValue(), null);
    ...
    public ListNode<String> insertInOrder( ListNode<String> head, String s )
    {
        if (head == null) {
            ...
            ListNode<String> node = head;
            ListNode<String> save = node;
            ListNode<String> prev = null;
            int count = 0;
            while (node != null) {
                if (s.compareTo(node.getValue()) < 0) {
                    if (count == 0) {
                        ListNode<String> string = new ListNode<String>(s, node);
                        save = string;
                    }
                    else {
                        ListNode<String> string = new ListNode<String>(s, node);
                ...
                else if (s.equals(node.getValue())) {
                    break;
                }
     ...
     */
    public static void sort( ListNode<String> top_head )
    {
        if (top_head != null) {
            ListNode<String> head = new ListNode<String>(top_head.getValue(), top_head.getNext());
            ListNode<String> node = head;
            ListNode<String> start = head;
            ListNode<String> least = head;
            ListNode<String> prev = null;
            ListNode<String> addTo = null;
            boolean newHead = false;
            while (true) {
                while (node.getNext() != null) {
                    if (node.getNext().getValue().compareTo(least.getValue()) < 0) {
                        prev = node;
                        least = node.getNext();
                    }
                    node = node.getNext();
                ...
                System.out.println( strList( head ) );
                node = start;
                least = start;
                prev = null;
            }
            top_head.setValue(head.getValue());
            top_head.setNext(head.getNext());
            System.out.println( strList( top_head ) );
        }
    }
    ...
    public static ListNode2<Integer> removeMax( ListNode2<Integer> head )
    {
        ...
        ListNode2<Integer> largest = node;
        while (node.getNext() != null) {
            if (node.getNext().getValue() > largest.getValue()) {
                largest = node.getNext();
            }
            node = node.getNext();
        }
        if (largest == head) {
            ...
            ListNode2<Integer> next = largest.getNext();
            ListNode2<Integer> prev = largest.getPrevious();
            if (next != null) {
        ...
        do
        {
            ...
            if ( response.length() > 0 )
            {
                ...
                switch ( response.charAt( 0 ) )
                {
                    ...
                    case 'D':
                    case 'd':
                        ListNode<String> headD = buildList( new String[] { "B", "F", "K", "Q", "Ant", "Aardvark", "apple" } );
