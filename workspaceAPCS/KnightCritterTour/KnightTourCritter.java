...
public class KnightTourCritter extends KnightCritter
{
    ...
    public Location getBestMove( ArrayList<Location> locs )
    {
        ...
        for (Location loc : locs) {
            if (accessGrid[loc.getRow()][loc.getCol()] < accessGrid[bestMove.
                getRow()][bestMove.getCol()]) {
                bestMove = loc;
            }
        }
    ...
    {
        int[][] moves = {{1, 2, 2, 1, -1, -2, -2, -1}, {-2, -1, 1, 2, 2, 1, -1, 
            -2}};
        for (int i = 0; i < 8; i++) {
            if (loc.getRow() + moves[1][i] < accessGrid.length && loc.getRow() +
                moves[1][i] >= 0 && loc.getCol() + moves[0][i] < accessGrid[0].
                length && loc.getCol() + moves[0][i] >= 0) {
                accessGrid[loc.getRow() + moves[1][i]][loc.getCol() + moves[0][i
                ]] -= 1;
            }
        }
    }
