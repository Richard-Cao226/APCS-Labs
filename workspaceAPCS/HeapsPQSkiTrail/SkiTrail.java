...
public class SkiTrail
{
    ...
    {
        sk.setTime(sk.time() + (length / (sk.skillLevel() * (rand.nextDouble() + 1))));
        skierPQ.add(sk);
    }
    ...
    {
        TreeSet skierSet = new TreeSet();
        PriorityQueue<Skier> tempPQ = skierPQ;
        PriorityQueue<Skier> skierLeftPQ = new PriorityQueue<Skier>();
        for (Skier sk : tempPQ) {
            if (sk.time() < clockTime) {
                skierSet.add(skierPQ.remove());
            } else {
                skierLeftPQ.add(skierPQ.remove());
            }
        }
        skierPQ = skierLeftPQ;
        return skierSet;
