...
public class MyTreeSet<E>
{
    ...
    private int width( TreeNode<E> root )
    {
        if (root == null) {
            ...
            return Math.max(Math.max(width(root.getLeft()), width(root.getRight())), height(root.getLeft()) + height(root.getRight()) + 1);
        }
    }
    ...
    private int[] nodeCounts( TreeNode<E> root )
    {
        ...
        if (root == null) {
            ...
            result[1] = Math.abs(countNodesHelper(root.getLeft()) - countNodesHelper(root.getRight()));
            return result;
        }
    }
    ...
    private void accumulate( TreeNode<Integer> root )
    {
        if (root == null || root.getRight() == null && root.getLeft() == null) {
            ...
            accumulate(root.getLeft());
            accumulate(root.getRight());
            root.setValue(root.getLeft().getValue() + root.getRight().getValue() + root.getValue());
        }
    }
