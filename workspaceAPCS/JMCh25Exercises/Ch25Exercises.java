...
public class Ch25Exercises
{
    ...
    {
        completeTree[0] = (Integer) completeTree[0] - ((Integer) completeTree[0] + 1) / 2;
    }
    ...
    public boolean isHeap( Integer[] items, int numNodes )
    {
        int i = 1;
        int j = i * 2;
        while (i < items.length && j < items.length && items[i] < items[j]) {
            i++;
            j = i * 2;
        }
        return j >= numNodes;
    ...
    {
        int i = 1;
        while (items[i] > items[i * 2] || items[i] > items[i * 2 + 1]) {
